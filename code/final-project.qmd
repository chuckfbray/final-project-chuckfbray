...
The first wave occurred roughly March 2020 to June 6, 2020, which we classify as the "Originator" wave. This was succeeded by the Second and Third waves, which occurred `r casewavedef$begindate[2]` to `r as.Date(casewavedef$enddate[2])` and `r casewavedef$begindate[3]` to `r as.Date(casewavedef$enddate[3])`, respectively. The fourth wave, which we can call the Delta wave based on known speciation (@Deltatime), occurred `r casewavedef$begindate[4]` to `r as.Date(casewavedef$enddate[4])`. This was eventually followed by the extremely transmissible Omicron variant, which characterized the wave lasting `r casewavedef$begindate[6]` to `r as.Date(casewavedef$enddate[6])`. Finally, while there continued to be local minima and maxima that could be considered to delineate further waves, it was decided that the low number of deaths along with moderation in number of hospitalizations, and regional segmentation in terms of dispersion around nationwide wave end dates (see Figure 2), collectively characterized a "post-acute" phase of the pandemic, starting at `r as.Date(casewavedef$enddate[7])` and extending through the end of data collection, December 1, 2024. This was driven by vaccination efforts and milder Omicron subvariants prevalent at the time and to this day. (@omicron). \newline

![Dispersion of regional minima and maxima about national wave end dates (cases)]("../figs/Figure2.png"){fig-align="center"}
...
![State-level mortality by wave, regionally grouped]("../figs/Figure3.png"){fig-align="center"}
...
The predominant strain of COVID-19 that characterized each wave became less virulent, comparing earlier waves to later waves. The original wave resulted in a mortality : case ratio of `r round(virulence$deathpercase[1],2)` while the first wave of Omicron exhibited a ratio of `r round(virulence$deathpercase[6],2)`. By the approaching-endemicity phase of the pandemic, which we call post-acute, the ratio had become `r round(virulence$deathpercase[7],2)` when observing the period March 26, 2022 to October 29, 2022, for which data reporting is robust. The corresponding hospitalization : case rate ratios were, respectively, `r round(virulence$deathperhosp[1],2)`, `r round(virulence$deathperhosp[6],2)`, and `r round(virulence$deathperhosp[7],2)`. See Figure 4 below for trendlines of virulence.

![COVID-19 virulence by wave]("../figs/Fig4.png"){fig-align="center"}
...
Limitations
Several limitations of this study warrant discussion. First, the reliance on CDC-reported data may introduce biases related to underreporting or inconsistencies in data collection across states and time periods. Second, the study's wave-detection algorithm, while robust, is inherently sensitive to the choice of parameters (e.g., window size) and may not capture subtle transitions between waves. Third, this analysis does not directly incorporate external factors such as policy changes, socioeconomic variables, or mobility data, which could further contextualize the observed patterns.\newline
Future Directions
Future research should aim to refine the wave-detection methodology by integrating real-time data streams and machine learning techniques to enhance predictive capabilities. Additionally, longitudinal studies that incorporate demographic and behavioral data could provide deeper insights into the drivers of regional disparities. Finally, examining the impact of specific public health interventions and vaccination campaigns on wave dynamics would offer valuable lessons for managing future pandemics.
