...
deathwaves <- wavedat %>% filter(!is.na(transmax) & transmax>as.Date("2020-03-01") & outcome=="Deaths") %>% select(transmax,outcome) %>%
  mutate(begindate = lag(transmax, default=as.Date("2019-12-31"))+1) %>% rename(enddate = transmax)

hospwaves <- wavedat %>% filter(!is.na(transmax) & transmax>as.Date("2020-03-01") & outcome=="Hospitalizations") %>% select(transmax,outcome) %>%
  mutate(begindate = lag(transmax, default=as.Date("2019-12-31"))+1) %>% rename(enddate = transmax)
...
# boxplots
ggplot(variation_min %>% filter(outcome!="Deaths" #& begindate<as.Date("2022-04-10")
                                )# %>%
       # mutate(begindate=paste0("Wave starting ",begindate))
       ,
